locals{
    ingress_cidr_blocks = {for k,r in var.ingress_rules: k => r if length(coalesce(r.cidr_blocks,[])) > 0}
    egress_cidr_blocks =  {for k,r in var.egress_rules: k => r if length(coalesce(r.cidr_blocks,[])) > 0}
    
    ingress_self_rules = {for k,r in var.ingress_rules: k => r if r.self == true }
    egress_self_rules = {for k,r in var.egress_rules: k => r if r.self == true}

    ingress_source_security_group_ids = {for k,r in var.ingress_rules: k => r if r.security_group_id != null}
    egress_source_security_group_ids = {for k,r in var.egress_rules: k => r if r.security_group_id != null}
}
